...
class L3tumPrometheusBundleTest extends WebTestCase
{
    public function testSearchProductsActionSuccess()
    {
        ...
        self::assertContains(
            'symfony_http_2xx_responses_total{action="GET-app_dummy_homepage",status="200",uri="http://:/"} 1'.PHP_EOL,
            $content
        );
        self::assertContains(
        ...
        self::assertContains(
            'symfony_http_requests_total{action="GET-app_dummy_homepage",uri="http://:/"} 1'.PHP_EOL,
            $content
        );
        self::assertContains(
